        ...
        try {
            ...
            if (Array.isArray(parsedData) && parsedData.length > 0) {
                ...
                try {
                    ...
                    console.warn("Deck too large for LocalStorage. Running in-memory.", storageError);
                }
...
function setupFilters() {
    const domains = [...new Set(allFlashcards.map(card => card.domain).filter(Boolean))];
...
function filterCardsAndRefreshDeck() {
    ...
    const domainCards = selectedDomain === 'all' 
        ? [...allFlashcards] 
...
function showQuestion(index) {
    ...
    if (!activeFlashcards || activeFlashcards.length === 0 || index >= activeFlashcards.length) {
        if (allFlashcards.length > 0) {
            ...
            questionText.textContent = "Please upload a valid JSON file to get started.";
        }
    ...
    if (currentCard.options && Array.isArray(currentCard.options)) {
        currentCard.options.forEach(optionText => {
            const button = document.createElement('button');
            button.className = 'option-btn';
            button.textContent = optionText;
            
            button.addEventListener('click', () => handleOptionSelection(button, currentCard));
...
function handleOptionSelection(selectedButton, currentCard) {
    ...
    const chosenText = selectedButton.textContent.trim();
    const correctText = currentCard.correct_answer.trim();
...
function updateProgress() {
    ...
    progressTracker.textContent = `Cards remaining: ${remaining > 0 ? remaining.toLocaleString() : 0}`;
}